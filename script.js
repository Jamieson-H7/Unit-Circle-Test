...
function refreshTextSize() {
  ...
  document.body.classList.add('text-size-measuring');
  restoreScaledText();
  void document.body.offsetWidth;
  scalePageText(TEXT_SIZE_SCALES[selectedTextSize]);
  void document.body.offsetWidth;
  document.body.classList.remove('text-size-measuring');
}
...
function scaleNewSvgText(root) {
  const scale = TEXT_SIZE_SCALES[selectedTextSize];
  if (scale === 1) return;
  [...root.querySelectorAll('text')].forEach(element => {
    if (scaledTextElements.has(element)) return;
    const computedSize = Number.parseFloat(getComputedStyle(element).fontSize);
    if (!Number.isFinite(computedSize) || computedSize <= 0) return;
    scaledTextElements.set(element, element.style.fontSize);
    element.style.fontSize = `${computedSize * scale}px`;
  });
}
...
function buildQuizQuestions() {
  ...
  STANDARD_ANGLES.forEach(angle => {
    ...
    [['cos', 0, 'Cosine is the x-coordinate'], ['sin', 1, 'Sine is the y-coordinate'], ['tan', 2, 'Tangent is sin θ / cos θ']].forEach(([category, valueIndex, prefix]) => {
      ...
      questions.push({ id: `${category}-${angle}`, category, topic: LABELS[category], question: `What is ${category} ${ANGLE_NAMES[angle]}?`, ...answerSet(value, distractors), answer: value, angle, angleUnit:'degrees', explanation: `${prefix}. At ${ANGLE_NAMES[angle]}, the value is ${value}.` });
      questions.push({ id: `${category}-radians-${angle}`, category, topic: LABELS[category], question: `What is ${category} ${radianName(angle)}?`, ...answerSet(value, distractors), answer: value, angle, angleUnit:'radians', explanation: `${prefix}. At ${radianName(angle)}, the value is ${value}.` });
    });
  });
  STANDARD_ANGLES.forEach(angle => {
...
function questionsForTopic(topic, ratioUnit = 'both') {
  const questions = buildQuizQuestions();
  if (topic === 'all') return questions;
  if (topic !== 'ratios') return questions.filter(item => item.category === topic);
  return questions.filter(item => isRatioQuestion(item) && (ratioUnit === 'both' || ratioQuestionUnit(item) === ratioUnit));
}
function isRatioQuestion(question) { return ['sin', 'cos', 'tan'].includes(question.category); }
function ratioQuestionUnit(question) { return question.angleUnit || (question.id.includes('-radians-') ? 'radians' : 'degrees'); }
function selectedRatioUnit() { return document.querySelector('#ratio-angle-unit').value; }
function showTopicPreview(topic) {
  const ratioUnit = topic === 'ratios' ? selectedRatioUnit() : 'both';
  const questions = questionsForTopic(topic, ratioUnit);
  const sampleAngles = { all:0, signs:90, locations:135, ratios:225 };
  const sample = questions.find(question => question.angle === sampleAngles[topic]) || questions[0];
  const previews = { all:'Take a tour around the whole circle with quadrant signs, degree-and-radian matches, and exact sine, cosine, and tangent values.', signs:'Read the circle by its quadrants: identify where angles land and how cosine, sine, and tangent change sign.', locations:'Practice placing angles precisely by matching degrees with radians and finding their locations on the circle.', ratios:'Build fluency with exact values by working through sine, cosine, and tangent at standard angles.' };
  const unitDescriptions = { both:'degree and radian angles', degrees:'degree angles', radians:'radian angles' };
  previewTopic = topic;
  document.querySelector('#ratio-unit-picker').hidden = topic !== 'ratios';
  document.querySelector('#preview-topic').textContent = topic === 'all' ? 'ALL TOPICS' : topic === 'ratios' ? 'RATIOS' : LABELS[topic];
  document.querySelector('#preview-title').textContent = `${questions.length} questions ready`;
  document.querySelector('#preview-copy').textContent = `${previews[topic]} Question types: ${topic === 'all' ? 'quadrants and signs; degrees and radians; sine, cosine, and tangent values.' : topic === 'signs' ? 'quadrant locations, individual cosine and sine signs, and tangent signs.' : topic === 'locations' ? 'degree-to-radian matches and angle locations.' : `exact sine, cosine, and tangent values using ${unitDescriptions[ratioUnit]}.`} Sample: ${sample.question}`;
  document.querySelector('#topic-preview').hidden = false;
  document.querySelector('.practice-layout').hidden = true;
  document.querySelectorAll('#topic-tabs button').forEach(tab => tab.classList.toggle('active', tab.dataset.topic === topic));
}
function startTopicQuiz(topic, randomize = false, ratioUnit = 'both') { rememberQuizSession(); activeQuestions = questionsForTopic(topic, ratioUnit); if (randomize) activeQuestions = shuffle(activeQuestions); quizIndex = 0; quizScore = 0; missedQuestionIds = []; previewTopic = null; document.querySelector('#topic-preview').hidden = true; document.querySelector('.practice-layout').hidden = false; document.querySelectorAll('#topic-tabs button').forEach(tab => tab.classList.toggle('active', tab.dataset.topic === 'active')); renderQuiz(); rememberQuizSession(); }
function startQuiz({ preserve = false, focusCategories = [], includedCategories = null, maintenance = false, ratioUnits = null } = {}) {
  const order = focusCategories.length ? [...focusCategories, 'locations', 'signs', 'sin', 'cos', 'tan'] : ['signs', 'locations', 'sin', 'cos', 'tan'];
  const categories = includedCategories || ['signs', 'locations', 'sin', 'cos', 'tan'];
  let generated = shuffle(buildQuizQuestions()).filter(question => categories.includes(question.category));
  if (ratioUnits?.length) generated = generated.filter(question => !isRatioQuestion(question) || ratioUnits.includes(ratioQuestionUnit(question)));
  if (maintenance) generated = generated.slice(0, 5);
  activeQuestions = generated.sort((a, b) => order.indexOf(a.category) - order.indexOf(b.category));
  quizIndex = preserve ? quizIndex : 0;
  quizScore = preserve ? quizScore : 0;
  missedQuestionIds = preserve ? missedQuestionIds : [];
  showScreen('quiz');
  renderQuiz();
  rememberQuizSession();
}
...
const LEARN_LESSONS = {
  ...
  angles: {
    ...
    usesMathMarkup: true,
    body: [
      'Degrees split a full turn into 360 equal parts. Half a turn is <span class="learn-math" role="math" aria-label="180 degrees">180&deg;</span>, and a quarter-turn is <span class="learn-math" role="math" aria-label="90 degrees">90&deg;</span>.',
      'Radians use the radius as a ruler along the circle’s edge. An arc one radius long makes an angle of <span class="learn-math" role="math" aria-label="1 radian">1<span class="learn-math-unit">radian</span></span>. On a unit circle, the radian measure is simply the length of that arc.',
      'The whole edge is <span class="learn-math" role="math" aria-label="2 pi times the radius">2&pi;<i>r</i></span> long, so <span class="learn-math" role="math" aria-label="360 degrees equals 2 pi radians">360&deg;&nbsp;=&nbsp;2&pi;<span class="learn-math-unit">rad</span></span>. This gives the conversion <span class="learn-math learn-math-equation" role="math" aria-label="degrees over 180 degrees equals radians over pi"><span class="learn-fraction"><span>degrees</span><span>180&deg;</span></span><span>=</span><span class="learn-fraction"><span>radians</span><span>&pi;</span></span></span>.'
    ],
    takeaway: 'Same turn, different units: <span class="learn-math" role="math" aria-label="180 degrees equals pi radians">180&deg;&nbsp;=&nbsp;&pi;<span class="learn-math-unit">rad</span></span>.'
  },
  sincos: {
    angle: 45,
...
function renderPretest() { const item = window.pretestQuestions[pretestIndex]; document.querySelector('#pretest-topic').textContent = item.topic; document.querySelector('#pretest-question').textContent = item.question; document.querySelector('#pretest-progress').textContent = `${String(pretestIndex + 1).padStart(2, '0')} / ${window.pretestQuestions.length}`; document.querySelector('#pretest-progress-bar').style.width = `${((pretestIndex + 1) / window.pretestQuestions.length) * 100}%`; document.querySelector('#pretest-feedback').textContent = ''; const answers = document.querySelector('#pretest-answers'); answers.innerHTML = ''; item.answers.forEach((answer, index) => { const button = document.createElement('button'); button.className = 'answer-button'; button.textContent = `${String.fromCharCode(65 + index)}  ${answer}`; button.addEventListener('click', () => answerPretest(index, button)); answers.appendChild(button); }); scheduleTextSizeRefresh(); }
...
function renderQuiz() {
  ...
  nextButton.firstChild.nodeValue = 'Choose an answer ';
...
function renderCorrectCircleHighlight(item) {
  ...
  if (item.category === 'signs') {
    ...
    const labelX = item.category === 'cos' ? (projection.x + 50) / 2 : 56;
    const labelY = item.category === 'cos' ? 43 : (projection.y + 50) / 2;
    const labelAnchor = item.category === 'sin' ? 'start' : 'middle';
...
document.querySelector('#retry-quiz').addEventListener('click', () => {
  const missedQuestions = activeQuestions.filter(question => missedQuestionIds.includes(question.id));
  const categories = [...new Set(missedQuestions.map(question => question.category))];
  const ratioUnits = [...new Set(missedQuestions.filter(isRatioQuestion).map(ratioQuestionUnit))];
  startQuiz({ focusCategories:categories, includedCategories:categories.length ? categories : null, maintenance:categories.length === 0, ratioUnits });
});
...
document.querySelector('#ratio-angle-unit').addEventListener('change', () => { if (previewTopic === 'ratios') showTopicPreview('ratios'); });
document.querySelector('#start-topic-quiz').addEventListener('click', () => { if (previewTopic) startTopicQuiz(previewTopic, false, previewTopic === 'ratios' ? selectedRatioUnit() : 'both'); });
document.querySelector('#randomize-topic-quiz').addEventListener('click', () => { if (previewTopic) startTopicQuiz(previewTopic, true, previewTopic === 'ratios' ? selectedRatioUnit() : 'both'); });
